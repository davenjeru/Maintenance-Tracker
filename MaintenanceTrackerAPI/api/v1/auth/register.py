...
from MaintenanceTrackerAPI.api.v1.boilerplate import get_validated_payload, extract_from_payload, generate_auth_output, \
    PayloadExtractionError
from MaintenanceTrackerAPI.api.v1.models.user_model import Consumer, Admin, UserTransactionError
...
definition = dict(email=fields.String(title='Your email address', required=True, example='myemail@company.com'),
                  ...
                  security_answer=fields.String(title='Your security answer', required=True,
                                                example='company'),
                  role=fields.String(title='Your role', required=None, example=None))
...
class Register(Resource):
    ...
    def post(self):
        ...
        8. If one wants to create an administrator account, add a role field with the value set as 'Administrator'.
        It defaults to None

        """

        # This is a short cut for setting all these values to None with minimal code
        email, password, confirm_password, security_question, security_answer, role = tuple('     '.split(' '))

        try:
            payload = get_validated_payload(self)
            list_of_names = ['email', 'password', 'confirm_password', 'security_question', 'security_answer', 'role']
        ...
        try:
            if role is None or role == 'Consumer':
                created_user = Consumer(email, password, security_question, security_answer)
            elif role == 'Administrator':
                created_user = Admin(email, password, security_question, security_answer)
            else:
